...
old = '''  const applyReceiptAffiliate = (affiliate: Affiliate | null) => {
    if (!affiliate) return;
...
new = '''  const applyReceiptAffiliate = (affiliate: Affiliate | null) => {
    if (!affiliate) return;
    setReceiptForm((current) => {
...
if text.count(old) != 1:
    raise SystemExit(f"Bloque applyReceiptAffiliate inesperado: {text.count(old)} coincidencias")
text = text.replace(old, new)

old2 = '''  const updateReceiptPaidMonth = (month: string, checked: boolean) => {
    setReceiptForm((current) => {
...
new2 = '''  const updateReceiptPaidMonth = (month: string, checked: boolean) => {
    setReceiptForm((current) => {
      const paidMonths = checked
        ? uniqueSorted([...current.paidMonths, month])
        : current.paidMonths.filter((item) => item !== month);
...
if text.count(old2) != 1:
    raise SystemExit(f"Bloque updateReceiptPaidMonth inesperado: {text.count(old2)} coincidencias")
